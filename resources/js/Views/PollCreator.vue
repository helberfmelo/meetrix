<template>
    <div class="max-w-2xl mx-auto space-y-6 sm:space-y-8">
        <div class="flex items-center space-x-3 sm:space-x-4">
            ...
            <h1 class="text-2xl sm:text-3xl font-black text-gray-900">New Meeting Poll</h1>
        </div>

        <div class="bg-white p-5 sm:p-8 rounded-3xl shadow-xl border border-gray-100 space-y-6">
            <div class="space-y-4">
                <div class="space-y-1">
            ...
            <div class="space-y-4">
                ...
                <div v-for="(option, index) in poll.options" :key="index" class="flex items-start space-x-3 group">
                    ...
                    <button @click="removeOption(index)" v-if="poll.options.length > 1" class="mt-2 sm:mt-0 text-gray-300 hover:text-red-500 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-all">✕</button>
                </div>
            </div>
