<template>
    <div class="space-y-12 animate-in fade-in duration-1000">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
                <h1 class="text-5xl font-black text-zinc-950 dark:text-white tracking-tighter uppercase font-outfit">{{ $t('dashboard.title') }}<span class="text-meetrix-orange">.INSIGHTS</span></h1>
                <p class="text-slate-500 font-bold text-xs uppercase tracking-[0.4em] mt-2">{{ $t('dashboard.insights_subtitle') }}</p>
            </div>
            <div class="px-6 py-3 bg-white dark:bg-zinc-900 border border-black/5 dark:border-white/5 rounded-full text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-3 shadow-sm">
        ...
        <div class="grid grid-cols-1 xl:grid-cols-12 gap-12">
            ...
            <div class="xl:col-span-8 space-y-12">
                ...
                <div v-if="loadingStats" class="grid grid-cols-1 md:grid-cols-3 gap-8 animate-pulse">
                    <div v-for="i in 3" :key="i" class="h-40 bg-white dark:bg-zinc-900 rounded-5xl border border-black/5 dark:border-white/5"></div>
                </div>
                <div v-else class="grid grid-cols-1 md:grid-cols-3 gap-8">
                    <div class="group relative bg-white dark:bg-zinc-900 p-8 rounded-5xl border border-black/5 dark:border-white/5 hover:border-meetrix-orange/30 transition-all duration-500 overflow-hidden shadow-premium">
                        ...
                        <h3 class="text-slate-500 text-[10px] font-black uppercase tracking-[0.3em] mb-4">{{ $t('dashboard.total_bookings') }}</h3>
                        <p class="text-6xl font-black text-zinc-950 dark:text-white tracking-tighter">{{ stats.total_bookings }}</p>
                    ...
                    <div class="group relative bg-white dark:bg-zinc-900 p-8 rounded-5xl border border-black/5 dark:border-white/5 hover:border-meetrix-green/30 transition-all duration-500 overflow-hidden shadow-premium">
                        <h3 class="text-slate-500 text-[10px] font-black uppercase tracking-[0.3em] mb-4">{{ $t('dashboard.active_pages') }}</h3>
                        <p class="text-6xl font-black text-zinc-950 dark:text-white tracking-tighter">{{ stats.active_pages }}</p>
                        <div class="mt-6 flex items-center gap-2 text-[10px] font-black text-slate-500 uppercase tracking-widest">
                            {{ $t('dashboard.pages_online_tag') }}
                        </div>
                    </div>

                    <div class="group relative bg-zinc-50 dark:bg-zinc-950 p-8 rounded-5xl border border-black/5 dark:border-white/5 hover:border-white/10 transition-all duration-500 overflow-hidden shadow-premium">
                        ...
                        <h3 class="text-slate-500 text-[10px] font-black uppercase tracking-[0.3em] mb-4">{{ $t('dashboard.revenue_engine') }}</h3>
                        <p class="text-6xl font-black text-meetrix-orange tracking-tighter">PRO</p>
                        <div class="mt-6 flex items-center gap-2">
                        ...
                        <div v-for="(stage, index) in funnelData" :key="index" class="group relative flex items-end gap-8">
                            <div class="flex-1">
                                ...
                                <div class="h-16 bg-zinc-50 dark:bg-zinc-950 rounded-2xl p-1 overflow-hidden shadow-inner relative">
                                    <div class="h-full bg-gradient-to-r transition-all duration-1000 ease-out flex items-center px-6"
                                        :class="stage.color"
            ...
            <div class="xl:col-span-4 space-y-8">
                <div class="bg-white dark:bg-zinc-950 rounded-5xl border border-black/5 dark:border-white/5 p-10 h-full shadow-premium">
                    ...
                    <div v-else class="space-y-4">
                        <div v-for="page in pageStore.pages" :key="page.id" 
                            class="group p-6 rounded-4xl border border-transparent bg-zinc-50 dark:bg-zinc-900/50 hover:bg-zinc-100 dark:hover:bg-zinc-900 hover:border-black/5 dark:hover:border-white/5 transition-all duration-500 relative overflow-hidden">
                            ...
                            <div class="flex justify-between items-start mb-6">
                                <div class="min-w-0">
                                    <h3 class="text-lg font-black text-zinc-950 dark:text-white uppercase tracking-tighter truncate group-hover:text-meetrix-orange transition-colors">{{ page.title }}</h3>
                            ...
                            <div class="flex justify-between items-center pt-6 border-t border-black/5 dark:border-white/5 text-[9px] font-black uppercase tracking-[0.2em] text-slate-500">
        ...
        <div class="py-12 border-t border-black/5 dark:border-white/5 flex justify-between items-center text-[8px] font-black text-slate-400 dark:text-slate-600 uppercase tracking-[0.5em]">
            <div>MEETRIX // CORE_ENGINE_STATUS: 200_OK</div>
