# Task Status - Phase 9

## Concluído [x]
- [x] Corrigir lógica de validação de cupons (CouponController/SubscriptionController)
- [x] Corrigir bugs de estatísticas no Dashboard (DashboardController)
- [x] Padronizar colunas do banco de dados (`scheduling_page_id`)
- [x] Implementar SoftDeletes na tabela de Bookings
- [x] Garantir Green Suite localmente (`CouponTest` e `DashboardTest`)
- [x] Organizar documentação na pasta `docs/`

## Em Andamento [/]
- [/] Restaurar execução de PHP no servidor de produção (Bug de .htaccess)
- [/] Sincronizar banco de dados de produção (Migrate nuclear)

## Pendente
- [ ] Validar cadastro de novo cliente em produção
- [ ] Validar cupom de 100% OFF (cupom100) em produção
- [ ] Validar redirecionamento e dados do Dashboard em produção
