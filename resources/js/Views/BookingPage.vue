<template>
    <div class="min-h-screen bg-gray-50/50 py-12 px-4 sm:px-6 lg:px-8" :style="brandFontStyle">
        ...
        <div v-else-if="page" class="max-w-5xl mx-auto">
            <!-- Booking Container -->
            <div class="bg-white rounded-2xl shadow-2xl overflow-hidden flex flex-col md:flex-row min-h-[650px] border border-gray-100">
                
                <!-- Left Sidebar: Page Details -->
                <div class="w-full md:w-80 bg-slate-900 p-10 text-white flex flex-col justify-between" :style="sidebarStyle">
                    <div>
                        <!-- Brand Logo -->
                        ...
                        <h1 class="text-2xl font-black leading-tight uppercase font-outfit tracking-tighter">{{ page.title }}</h1>
                        <p class="mt-4 text-slate-400 leading-relaxed font-medium text-sm">{{ page.intro_text }}</p>
                ...
                <div class="flex-1 p-10 flex flex-col bg-white">
                    ...
                    <!-- Progress Bar -->
                    <div class="mb-10 w-full flex space-x-2">
                    ...
                    <!-- Step 1: Select Type -->
                    <div v-if="step === 'type'" class="space-y-8 animate-in fade-in zoom-in-95">
                        <h2 class="text-3xl font-black text-slate-900 tracking-tighter uppercase">{{ $t('booking.select_service') }}</h2>
                        <div class="grid grid-cols-1 gap-4">
                            <button 
                                v-for="type in page.appointment_types" 
                                :key="type.id"
                                @click="selectType(type)"
                                class="flex items-center justify-between p-6 border-2 border-slate-50 rounded-2xl transition-all group active:scale-[0.98] hover:shadow-2xl hover:border-transparent"
                                :style="{ borderColor: hoverType === type.id ? primaryColor : '#f8fafc' }"
                                @mouseenter="hoverType = type.id"
                                @mouseleave="hoverType = null"
                            >
                                <div class="text-left">
                                    <h3 class="font-black text-slate-900 text-xl uppercase tracking-tighter">{{ type.name }}</h3>
                                    <p class="text-slate-400 text-[10px] font-black uppercase tracking-widest mt-1">{{ type.duration_minutes }} MIN // SYNC_OK</p>
                                </div>
                                <div class="text-right">
                                    <span v-if="type.price > 0" class="font-black text-2xl tracking-tighter" :style="{ color: primaryColor }">{{ formatCurrency(type.price) }}</span>
                                    <span v-else class="text-meetrix-green font-black text-xl tracking-tighter uppercase">{{ $t('home.price_free_value') }}</span>
                                </div>
                            </button>
                        </div>
                    </div>
                    ...
                    <div v-if="step === 'date'" class="space-y-8 animate-in fade-in slide-in-from-right-8">
                        <div class="flex items-center justify-between">
                            <button @click="step = 'type'" class="text-[10px] font-black uppercase tracking-[0.3em] text-slate-400 hover:text-slate-900 transition-colors">← {{ $t('common.back') }}</button>
                            <h2 class="text-3xl font-black text-slate-900 tracking-tighter uppercase">{{ $t('booking.agenda') }}</h2>
                        </div>
                        ...
                        <div class="flex flex-col lg:flex-row gap-8">
                            <div class="flex-1">
                                ...
                                <div class="grid grid-cols-4 lg:grid-cols-3 gap-2">
                                    <button 
                                        ...
                                        class="p-4 rounded-xl border-2 flex flex-col items-center transition-all"
                                    ...
                                    >
                                        ...
                                        <span class="text-xl font-black" :style="{ color: selectedDate === date ? 'white' : '#1e293b' }">{{ getDayNum(date) }}</span>
                                    </button>
                                </div>
                            </div>
                            ...
                            <div class="flex-1 min-h-[300px]">
                                ...
                                <div v-else-if="slots.length > 0" class="grid grid-cols-2 gap-2 overflow-y-auto max-h-[350px] pr-2 scrollbar-thin">
                                    <button 
                                        v-for="slot in slots" 
                                        :key="slot"
                                        @click="finishSelection(slot)"
                                        class="p-4 text-center rounded-xl border-2 font-black text-xs uppercase tracking-widest transition-all active:scale-95 hover:text-white"
                                        :style="{ 
                    ...
                    <div v-if="step === 'form'" class="space-y-8 animate-in fade-in slide-in-from-bottom-8">
                        <div class="flex items-center justify-between">
                            <button @click="step = 'date'" class="text-[10px] font-black uppercase tracking-[0.3em] text-slate-400 hover:text-slate-900 transition-colors">← {{ $t('common.back') }}</button>
                            <h2 class="text-3xl font-black text-slate-900 tracking-tighter uppercase">{{ $t('booking.your_data') }}</h2>
                        </div>
                        ...
                        <form @submit.prevent="submitBooking" class="space-y-6">
                            <div v-for="field in formFields" :key="field.name" class="space-y-2">
                                ...
                                <textarea 
                                    ...
                                    class="w-full px-6 py-4 rounded-2xl border-2 border-slate-50 focus:ring-4 focus:ring-slate-100 outline-none transition-all min-h-[120px] font-medium text-sm"
                                ...
                                <input 
                                    ...
                                    :required="field.required" 
                                    :type="field.type" 
                                    class="w-full px-6 py-4 rounded-2xl border-2 border-slate-50 focus:ring-4 focus:ring-slate-100 outline-none transition-all font-medium text-sm"
                                >
                            </div>
                            ...
                            <button 
                                type="submit" 
                                :disabled="submitting"
                                class="w-full py-6 px-8 rounded-2xl font-black text-sm uppercase tracking-[0.3em] text-white shadow-premium transition-all flex justify-center items-center group active:scale-95 disabled:bg-slate-300"
                    ...
                    <div v-if="step === 'success'" class="flex-1 flex flex-col items-center justify-center text-center animate-in zoom-in-90 duration-700">
                        ...
                        <router-link to="/" class="px-12 py-5 bg-zinc-950 text-white rounded-full font-black text-[10px] uppercase tracking-[0.4em] hover:scale-105 transition-all shadow-premium">
                            {{ $t('booking.back_home') }}
                        </router-link>
                    </div>
                </div>
